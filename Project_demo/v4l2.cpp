...
int init_dev (pass_data *s)
{
    ...
    //初始化设备
    init_device(s); //对摄像头数据的格式，图片大小
    //初始化映射
    init_mmap(s); // 对数据缓冲与队列中的映射建立
    //开始捕获摄像头数据
    start_capturing(s);
...
void init_device(pass_data* s)
{
    ...
    //通过系统io---ioctl来从系统中读取信息，并存放在结构体v4l2_capability
    ...
    cropcap.type = V4L2_BUF_TYPE_VIDEO_CAPTURE; //设置捕获摄像头数据
    ...
    s->fmt.fmt.pix.pixelformat	= V4L2_PIX_FMT_YUYV; //设置摄像头格式，根据自己的摄像头格式可以进行修改，比如mjpeg
    s->fmt.fmt.pix.field	= V4L2_FIELD_INTERLACED;

    //利用ioctl通过VIDIOC_S_FMT命令将配置好的信息，设置给linux中的v4l2
